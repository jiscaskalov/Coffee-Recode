...
public class DamageUtils {
    ...
    public static final RaycastFactory HIT_FACTORY = (context, blockPos) -> {
        ...
        return blockState.getCollisionShape(client.world, blockPos).raycast(context.start(), context.end(), blockPos);
    };
    ...
    public static float explosionDamage(LivingEntity target, Vec3d targetPos, Box targetBox, Vec3d explosionPos, float power, RaycastFactory raycastFactory) {
        ...
        return calculateReductions(damage, target, client.world.getDamageSources().explosion(null));
    }
    ...
    public static float getAttackDamage(LivingEntity attacker, LivingEntity target) {
        ...
        damage = calculateReductions(damage, target, attacker instanceof PlayerEntity player ? client.world.getDamageSources().playerAttack(player) : client.world.getDamageSources().mobAttack(attacker));
    ...
    public static float fallDamage(LivingEntity entity) {
        ...
        int surface = client.world.getWorldChunk(entity.getBlockPos()).getHeightmap(Heightmap.Type.MOTION_BLOCKING).get(entity.getBlockX() & 15, entity.getBlockZ() & 15);
        if (entity.getBlockY() >= surface) return fallDamageReductions(entity, surface);

        // Under the surface
        BlockHitResult raycastResult = client.world.raycast(new RaycastContext(entity.getPos(), new Vec3d(entity.getX(), client.world.getBottomY(), entity.getZ()), RaycastContext.ShapeType.COLLIDER, RaycastContext.FluidHandling.WATER, entity));
    ...
    private static float fallDamageReductions(LivingEntity entity, int surface) {
        ...
        return calculateReductions(fallHeight, entity, client.world.getDamageSources().fall());
    }
    ...
    public static float calculateReductions(float damage, LivingEntity entity, DamageSource damageSource) {
        if (damageSource.isScaledWithDifficulty()) {
            switch (client.world.getDifficulty()) {
